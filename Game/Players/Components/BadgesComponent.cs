...
namespace AuroraEmu.Game.Players.Components
{
    public class BadgesComponent : IDisposable
    {
        ...
        public void AddOrUpdateBadge(string achievementBadge, int checkLevel)
        {
            ...
            {
                Engine.Locator.BadgeController.Dao.UpdateBadge(badge.Id, achievementBadge + checkLevel);
                int slot = badge.Slot;
                int id = badge.Id;
                Badges.Remove(badge.Id);
                Badges.Add(id, new Badge(id, achievementBadge + checkLevel, slot));
            }
        }
    }
}